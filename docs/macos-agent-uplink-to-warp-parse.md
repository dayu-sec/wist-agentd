# macOS P0 日志采集与数据面上送方案（wist-agentd → WarpGateway data-plane）

## 1. 文档目的

定义在 **macOS** 主机上让 `wist-agentd` 采集高价值（P0）安全/审计数据，并通过 **TCP** 上送到
...
- `wist-agentd` 到 `warp-parse` 采用 **统一结构化 record（NDJSON）→ TCP(line framing)** 上送；
