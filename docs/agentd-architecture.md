...
这几类能力都不应被塞回 `wist-agentd`。

---

## 4. 顶层模块（当前实现）

`wist-agentd` 源码按职责域组织为 `src/` 下的目录模块（每个目录含 `mod.rs`，由 `lib.rs`
...
`wist-agentd` 维护独立于控制平面的本地执行状态。旧稿用单一线性列表把
...
`wist-agentd` 与 `wist-exec` 的关系应固定为：

- `wist-agentd` 是父进程与控制器
- `wist-exec` 是子进程与执行器
...
如果这几项不先冻结，`wist-agentd` 开发会很快陷入反复返工。
...
- `wist-agentd` 必须先于 `wist-exec` 进入开发主线
- `wist-agentd` 是边缘控制器，不是 step 执行器
- `wist-agentd` 必须持有本地状态机、队列和调度能力
- `wist-agentd` 与 `wist-exec` 通过独立本地协议交互
- `wist-agentd` 与 `wist-upgrader` 保持明确互斥与调度边界
