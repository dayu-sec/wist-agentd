# wist-agentd 故障处理与恢复设计

## 1. 文档目的

本文档定义 `wist-agentd` 在第一版里的故障分层、恢复语义和健康口径，重点回答三类问题：
...
`wist-agentd` 第一版必须明确以下 source-of-truth 关系。
...
`wist-agentd` 的 runtime health 不能只看 queue 和 running。
...
4. 正式配置必须只走统一 loader
5. 当前版本不暴露用户可调并发数，action 执行固定为单并发；兼容期内若历史配置仍显式声明并发值，必须拒绝大于 `1`
