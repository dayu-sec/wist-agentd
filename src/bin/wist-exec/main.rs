//! `wist-exec` 执行器（作为 `wist-agentd` 的同仓二进制构建）。
...
fn run() -> Result<(), String> {
    ...
    eprintln!("wist-exec finished in {}", workdir.root.display());
    Ok(())
}
...
mod tests {
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let suffix = SystemTime::now()
            ...
            .expect("duration")
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("wist-exec-{name}-{suffix}"));
    ...
    fn write_fixture_workdir(dir: &std::path::Path) {
        ...
        wist_shared::fs::write_json_atomic(&workdir.plan_path, &plan).expect("write plan");
        wist_shared::fs::write_json_atomic(&workdir.runtime_path, &runtime).expect("write runtime");
    }
