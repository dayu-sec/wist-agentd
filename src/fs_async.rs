//! Async filesystem helpers (tokio-based) local to `wist-agentd`.
...
mod tests {
    ...
    async fn write_and_read_json_round_trip() {
        let path = std::env::temp_dir().join(format!("wist-agentd-fs-async-{}", unique_suffix()));
    ...
    async fn write_bytes_private_atomic_sets_restrictive_permissions() {
        ...
        let dir =
            std::env::temp_dir().join(format!("wist-agentd-fs-async-perms-{}", unique_suffix()));
